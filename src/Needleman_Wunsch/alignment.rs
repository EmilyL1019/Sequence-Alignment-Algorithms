...
fn get_seq_char<'a>(cell: i32, seq1: &'a String, seq2: &'a String) -> (char, usize, char, usize) {
    ...
    return (seq1_char, seq1_char_index as usize, seq2_char, seq2_char_index as usize);
}

fn first_alignment<'a>(score_grid: &'a Vec<i32>, directions: &'a mut Vec<Direction>, seq1: String, seq2: String,) -> (Vec<char>, Vec<char>, Vec<Direction>){
...
fn is_new_path<'a>(directions: &'a mut Vec<Direction>, previous_alignment_directions: Vec<Direction>, seq2: String) -> (i32, i32) {
    let mut index: i32 = 0;
    ...
    while i >= 0 as i32 && (found_new_path == false){
        if previous_alignment_directions[i as usize] == Direction::Diagonal && directions.len() > index as usize + seq2.len() + 2 {
            println!("Index: {}", index);
        ...
        else if previous_alignment_directions[i as usize] == Direction::Up && directions.len() > index as usize + seq2.len() + 1{
            if (directions[index as usize + seq2.len() + 1] == Direction::DiagonalUpLeft) || (directions[index as usize + seq2.len() + 1] == Direction::UpLeft){
                found_new_path = true;
            }
            index = index + seq2.len() as i32 + 1;
        }
        else if previous_alignment_directions[i as usize] == Direction::Left && directions.len() > index as usize + 1{
            index = index + 1;
        }
        if !(found_new_path) {
            i = i - 1;
...
fn alignment_found<'a>(aligned_seq1: &'a mut Vec<Vec<char>>, aligned_seq2: &'a mut Vec<Vec<char>>, new_aligned_seq1: Vec<char>, new_aligned_seq2: Vec<char>) -> bool{
...
// Find best alignment
fn priv_best_alignment<'a>(score_grid: &'a Vec<i32>, directions: &'a mut Vec<Direction>, seq1: String, seq2: String, aligned_seq1: &'a mut Vec<Vec<char>>, aligned_seq2: &'a mut Vec<Vec<char>>) -> (Vec<String>, Vec<String>) {
    ...
    while new_path_index != -1 {
        ...
        if aligned_seq1.len() == 0 {
            (prev_aligned_seq1, prev_aligned_seq2, prev_aligned_directions) = first_alignment(score_grid, directions, seq1.clone(), seq2.clone());
            aligned_seq1.push(prev_aligned_seq1.clone());
            aligned_seq2.push(prev_aligned_seq2.clone());
            (new_path_index, grid_index) = is_new_path(directions, prev_aligned_directions.clone(), seq2.clone());       
        } 
        else {
            ...
            let seq1_1: String = new_aligned_seq1.to_vec().into_iter().collect();
            let seq2_1: String = new_aligned_seq2.to_vec().into_iter().collect();
            let (seq1_char, seq1_char_index, seq2_char, seq2_char_index) = get_seq_char(grid_index, &seq1, &seq2.clone());
            // Choose the new direction
            if (directions[grid_index as usize] == Direction::DiagonalUp || directions[grid_index as usize] == Direction::DiagonalUpLeft) && prev_aligned_directions[new_path_index as usize] == Direction::Diagonal{
            ...
            else if (directions[grid_index as usize] == Direction::DiagonalLeft && prev_aligned_directions[new_path_index as usize] == Direction::Diagonal) || ((directions[grid_index as usize] == Direction::DiagonalUpLeft || directions[grid_index as usize] == Direction::UpLeft) && prev_aligned_directions[new_path_index as usize] == Direction::Up) {
            ...
            while new_aligned_seq1.len() < prev_aligned_seq1.len() {
                let (seq1_char, seq1_char_index, seq2_char, seq2_char_index) = get_seq_char(grid_index, &seq1, &seq2);
                // If the current direction index includes Diagonal, add the two corresponding characters to the sequence strings    
                if (directions[grid_index as usize] == Direction::Diagonal) || (directions[grid_index as usize] == Direction::DiagonalLeft) || (directions[grid_index as usize] == Direction::DiagonalUp) || (directions[grid_index as usize] == Direction::DiagonalUpLeft) {
                    new_aligned_seq1.insert(0, seq1_char);
                    new_aligned_seq2.insert(0, seq2_char);
                ...
                // If the current direction index includes Up, add a gap to the first sequence and the corresponding character to the second sequence 
                else if (directions[grid_index as usize] == Direction::Up) || (directions[grid_index as usize ] == Direction::UpLeft) {
                    new_aligned_seq1.insert(0, seq1_char);
                    new_aligned_seq2.insert(0, '-');
                    new_aligned_directions.push(Direction::Up);
                    if (grid_index as i32 - seq2.len() as i32 - 1) as i32 >= 0 && seq1_char_index > 0 {
            ...
            if !(alignment_found(aligned_seq1, aligned_seq2, new_aligned_seq1.clone(), new_aligned_seq2.clone())) {
                ...
                (new_path_index, grid_index) = is_new_path(directions, new_aligned_directions.clone(), seq2.clone());
            }
...
// Find best alignment
pub fn build_best_alignment<'a>(score_grid: &'a Vec<i32>, directions: &'a mut Vec<Direction>, seq1: String, seq2: String) -> (Vec<String>, Vec<String>){
    let mut aligned_sequence1:Vec<Vec<char>> = vec![];
    let mut aligned_sequence2:Vec<Vec<char>> = vec![];
    return priv_best_alignment(score_grid, directions, seq1, seq2, &mut aligned_sequence1, &mut aligned_sequence2);
}
